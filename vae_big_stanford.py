...
train_data=CustomImageDataset(df,img_dir,transform,0,28672)
valid_data=CustomImageDataset(df,img_dir,transform,28672,28672+28416)
test_data=CustomImageDataset(df,img_dir,transform,57176,64088)
...
# %%
path="/DATA/chowdari1/saved_models/vae/vae_big_stanford_100ep.pth"
...
def plot_ae_outputs(encoder,decoder,n=5):
    ...
    for i in range(n):
      ...
      with torch.no_grad():
         rec_img  = decoder(encoder(img))
      plt.imshow(img.cpu().squeeze().numpy(), cmap='gist_gray')
      name='./images/stan_100_'+str(i)+'in.jpeg'      
      save_image(img, name)
      ax.get_xaxis().set_visible(False)
      ax.get_yaxis().set_visible(False)  
      if i == n//2:
      ...
      plt.imshow(rec_img.cpu().squeeze().numpy(), cmap='gist_gray')  
      plt.imshow(rec_img.cpu().squeeze().numpy(), cmap='gist_gray') 
      name='./images/stan_100_'+str(i)+'out.jpeg'      
...
plot_ae_outputs(vae.encoder,vae.decoder,n=5)
print('images added as: ./images/stan_100_')
